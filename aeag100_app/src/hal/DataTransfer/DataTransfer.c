...
s32 data_trans_init(s32 type)
{
	...
	if (ADC_DATA == type)
	{
		Status = data_trans_DmaInit(&adcDma, XPAR_AXI_DMA_0_DEVICE_ID);
		if (!Status)
		{
	...
	{
		Status = data_trans_DmaInit(&sRdmDma, XPAR_AXI_DMA_1_DEVICE_ID);
		if (!Status)
		{
			XGpio_Initialize(&sADCDMAGpio, XPAR_AXI_GPIO_DMA_DEVICE_ID);
			XGpio_SetDataDirection(&sADCDMAGpio, 1, 0);
...
s32 data_trans_set_callback(s32 type, DataTransCallback cb)
{
	...
	{
		pAxiDma = &adcDma;
		intID = XPAR_FABRIC_AXI_DMA_0_S2MM_INTROUT_INTR;
	...
	{
		pAxiDma = &sRdmDma;
		intID = XPAR_FABRIC_AXI_DMA_1_S2MM_INTROUT_INTR;
		handler = data_trans_RxIntrHandler;
		pRdmCallback = cb;
	}
