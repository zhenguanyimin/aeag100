...
#define FLASH_ADDR_VALID(val) (((val) >= 0) && ((val) < FLASH_VALIED_SIZE_16MB))
...
int32_t FlashHal_WriteByBytes(uint32_t flashAddr, uint8_t *writeBuf, uint32_t size)
{
    ...
    {
        ret = flash_memory_writeb(flashAddr, (const uint8_t *)writeBuf, size);
        if (ret != RET_OK)
        {
            goto out;
        }
    ...
    else
    {
        ...
        ret = flash_memory_writeb(flashAddr, (const uint8_t *)writeBuf, tail_len);
        if (ret != RET_OK)
        {
            goto out;
        }
        ...
        if (tail_len < size)
        {
            ret = flash_memory_writeb(flashAddr + tail_len, (const uint8_t *)writeBuf + tail_len, size - tail_len);
            if (ret != RET_OK)
            {
                goto out;
            }
