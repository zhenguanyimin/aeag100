...
static bool AlinkMsgProc_IsHeaderValid(alink_msg_head_t *header)
{
	bool ret = true;
	...
	// check the destid
	if (header->destid != DEV_AEAG)
	{
		LOG_VERBOSE("0x%x!=0x%x\r\n", header->destid, DEV_AEAG);
		ret = false;
		goto out;
	}
...
int32_t AlinkMsgSendRecv(void *pUartpl, alink_msg_t *pMsg, alink_ans_t *pAns_msg, uint32_t timeout)
{
	...
	if (AlinkMsgSend(pUartpl, pMsg))
	{
		...
		{
			ret = -2;
			if (AlinkMsgRecv_Pend(pAns_msg, timeout))
			{
				ret = -0;
			}
		}
...
//		//		}
//
//		checksum = AlinkMsgProc_GetHeaderChecksum((alink_msg_head_t *)getHitFreqHeader);
//		LOG_INFO("checksum = %x\r\n", checksum);
//		AlinkMsg_IsCrcValid((uint8_t *)getHitFreqCmd, 11, crc);
//		LOG_INFO("crc = %x\r\n", crc);
//		vTaskDelay(1000);
//		//		Timer_Callback();
//	}
