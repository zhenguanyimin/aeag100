...
uint8_t UpdataBatteryCapacityAndDisplayLedState(uint8_t original_data)
{
	...
	{
		uRecord = 0;
		msg.uLogNum = eLOG_ELECTRICITY;
		msg.Electricity_msg.uElectricity = remaincap;
		msg.Electricity_msg.uResult = ret;
		LoggingTask_Post(&msg);

		//		GetCurrent();
		if (GetSysWorkingMode() == 1)
		{
			msg.uLogNum = eLOG_HIT_CURRENT;
			msg.HitCurrent_msg.uTimestamp = (uint32_t)xTaskGetTickCount();
			msg.HitCurrent_msg.uCurrent = 12;
			LoggingTask_Post(&msg);
		}
	}
